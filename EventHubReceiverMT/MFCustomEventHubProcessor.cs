    ...
    public class MFCustomEventHubProcessor : PluggableCheckpointStoreEventProcessor<EventProcessorPartition>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Received events for partition {partition.PartitionId}");

                foreach (var currentEvent in events)
                {
                    processedEventCount++;
                    //Console.WriteLine($"Event: {currentEvent.EventBody}");
                    Console.WriteLine($"TaskIndex {m_taskNum} TaskId {Task.CurrentId} : Count:{processedEventCount},MsgId {currentEvent.MessageId},CorId {currentEvent.CorrelationId},Partition {partition.PartitionId} length {currentEvent.EventBody.ToArray().Length},eventBody={currentEvent.EventBody},eventsSinceLastChkpt:TODO,forceChkpt:TODO|Elapsed:{sw.ElapsedMilliseconds}|Consumed {DateTime.Now}");
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Exception while processing events: {ex}");
            }
        }
            ...
            try
            {
                if (partition != null)
                {
                    Console.Error.WriteLine(
                        $"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Exception on partition {partition.PartitionId} while " +
                        $"performing {operationDescription}: {exception}");
                }
                else
                {
                    Console.Error.WriteLine(
                        $"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Exception while performing {operationDescription}: {exception}");
                }
            }
            catch (Exception ex)
            {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                Console.WriteLine($"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Initializing partition {partition.PartitionId}");
            }
            catch (Exception ex)
            {
                // It is very important that you always guard against exceptions in
                ...
                Console.WriteLine($"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Exception while initializing a partition: {ex}");
            }
            sw.Start();
            return Task.CompletedTask;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                Console.WriteLine(
                    $"TaskIndex {m_taskNum} TaskId {Task.CurrentId} No longer processing partition {partition.PartitionId} " +
                    $"because {reason}");
                sw.Stop();
            }
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"TaskIndex {m_taskNum} TaskId {Task.CurrentId} Exception while stopping processing for a partition: {ex}");
            }
